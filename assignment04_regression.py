...
def plot_scatter_with_regression(
    ...
    # Scatter plot
    ax.scatter(plot_df[x_var], plot_df["ret"], alpha=0.5, s=30, label="Data")
    
    # Regression line
    intercept = model.params["Intercept"]
    slope = model.params[x_var]
    x_range = np.array([plot_df[x_var].min(), plot_df[x_var].max()])
    y_line = intercept + slope * x_range
    ax.plot(x_range, y_line, color="red", linewidth=2, label=f"Fitted line (slope={slope:.4f})")
    
    # Zoom to central data (2nd-98th percentiles)
    x_min = plot_df[x_var].quantile(0.02)
    x_max = plot_df[x_var].quantile(0.98)
...
def print_key_results(model, x_var: str) -> None:
    ...
    # Print results
    print(f"Intercept (β₀):     {intercept:10.6f}  (SE: {intercept_se:10.6f}, t: {intercept_tstat:8.4f}, p-value: {intercept_pval:8.4f})")
    print(f"Slope (β₁):         {slope:10.6f}  (SE: {slope_se:10.6f}, t: {slope_tstat:8.4f}, p-value: {slope_pval:8.4f})")
    print(f"\nR²:                 {r_squared:.6f}")
    print(f"Adjusted R²:        {adj_r_squared:.6f}")
    print(f"N observations:     {nobs}")
    
    # Significance test
    significant = "YES" if slope_pval < 0.05 else "NO"
    direction = "POSITIVE" if slope > 0 else "NEGATIVE"
    print(f"\nSlope is {direction} and SIGNIFICANT at 5%: {significant}")
    print("=" * 60 + "\n")
